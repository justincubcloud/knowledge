...
## 4. Chat Context Buffer — Auto-Compact (Phase B)
...
|---|---|---|---|
| — | Chat Context Buffer — Phase A (meter) | 2026-07-19 | Context % meter in ChatV2; `context_window` on `/llm-config`; relay `context` on ready/stream_end; `context_buffer` messagemeta. Phase B remains open as #4. Detail: `agent-chat-context-buffer.md` |
| — | Per-Message Agent Response Latency — Phase A | 2026-07-20 | Relay wall-clock `latency_ms` on `stream_end` + HMAC save; `response_latency` messagemeta; message API hydrate; ChatV2/guest bubble display. Phase B (TTFT/tools breakdown, admin p50/p95) still open in strategy. Detail: `agent-chat-message-response-latency.md` |
